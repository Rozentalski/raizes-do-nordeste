...
import { cadastrar, existeEmail } from '@/services/auth'
import {
  mascararCPF,
  mascararTelefone,
  validarCPF,
...
export default function Cadastro() {
  ...
  function handleSubmit(ev: FormEvent) {
    ...
    window.setTimeout(() => {
      ...
      const ok = cadastrar(novoUsuario, form.senha)
      setEnviando(false)
      if (!ok) {
  ...
  return (
    <main className="min-h-screen bg-background pb-12">
      <header className="sticky top-0 z-10 flex items-center bg-background/85 px-3 py-3 backdrop-blur">
