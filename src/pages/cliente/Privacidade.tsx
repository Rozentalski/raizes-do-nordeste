...
import { excluirConta, exportarDadosUsuario } from '@/services/lgpd'
...
export default function Privacidade() {
  const cookiesAnaliticos = useConsentimentosStore((s) => s.cookiesAnaliticos)
  const marketing = useConsentimentosStore((s) => s.marketing)
  const geolocalizacao = useConsentimentosStore((s) => s.geolocalizacao)
  const perfilConsumo = useConsentimentosStore((s) => s.perfilConsumo)
  const dataAtualizacao = useConsentimentosStore((s) => s.dataAtualizacao)
  const atualizar = useConsentimentosStore((s) => s.atualizar)
  ...
  return (
    <section className="space-y-6 px-4 py-6 md:px-8">
      ...
      <Dialog open={confirmar1} onOpenChange={setConfirmar1}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="font-serif">Excluir minha conta?</DialogTitle>
            <DialogDescription>
