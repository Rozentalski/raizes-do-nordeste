...
import { validarCupom } from '@/services/cuponsService'
...
export default function Carrinho() {
  ...
  const remover = useCarrinhoStore((s) => s.remover)
  const aplicarCupom = useCarrinhoStore((s) => s.aplicarCupom)
  const removerCupom = useCarrinhoStore((s) => s.removerCupom)
  const limpar = useCarrinhoStore((s) => s.limpar)
  const { subtotal, desconto, total } = useCarrinhoTotais()
  ...
  async function handleAplicarCupom() {
    ...
    try {
      const res = await validarCupom(codigoCupom)
      if (res.valido) {
    ...
    <section className="space-y-6 px-4 py-6 pb-32 md:px-8">
      ...
      {unidadeAtual && (
        <p className="rounded-lg border border-border bg-card px-4 py-3 text-sm text-foreground">
          Retirada em <strong>Raízes — {unidadeAtual.nome}</strong>
      ...
      <div className="fixed bottom-16 left-0 right-0 z-20 border-t border-border bg-card/95 px-4 py-3 backdrop-blur md:bottom-0">
