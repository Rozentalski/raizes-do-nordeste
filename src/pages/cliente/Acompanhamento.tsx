...
export default function Acompanhamento() {
  ...
  const pedido = useStorePedidos((s) => (id ? s.getPedido(id) : undefined))
...
function ResumoPedido({
  ...
  return (
    ...
    >
      <h3
        id="resumo-titulo"
        className="mb-3 font-serif text-lg text-foreground"
      >
        Resumo do pedido
      </h3>
        ...
        {pedido.itens.map((it) => (
          <li key={it.itemId} className="flex justify-between gap-2 text-sm">
            ...
            <span className="shrink-0 text-foreground">
              {formatarPreco(precoUnitario(it) * it.quantidade)}
            </span>
          </li>
        ))}
      </ul>
