...
export default function LoginAdmin() {
  ...
  const login = useUsuarioOperacionalStore((s) => s.login)

  const [email, setEmail] = useState('')
  const [senha, setSenha] = useState('')
  const [role, setRole] = useState<RoleOperacional>('atendente')
  const [mostrar, setMostrar] = useState(false)
  const [erro, setErro] = useState<string | null>(null)
  const [enviando, setEnviando] = useState(false)

  if (usuario) return <Navigate to={destinoPorRole(usuario.role)} replace />
  ...
  function handleSubmit(e: FormEvent) {
    ...
    window.setTimeout(() => {
      ...
      navigate(destinoPorRole(u.role), { replace: true })
    }, 400)
  }
    ...
    <div className="grid min-h-screen grid-cols-1 bg-[#3D2817] lg:grid-cols-2">
      ...
      <section className="relative flex flex-col items-center justify-center bg-background px-6 py-10">
            ...
            <div>
              ...
              <div className="relative mt-1.5">
                ...
                <input
                  ...
                  placeholder="seu.email@raizes.com"
        ...
        <p className="mt-6 text-xs text-muted-foreground">
          ...
          <a
            href="mailto:suporte@raizes.com"
...
function destinoPorRole(role: RoleOperacional): string {
