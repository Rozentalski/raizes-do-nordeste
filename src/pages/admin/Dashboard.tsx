...
const EQUIPE_ATIVA = [
  ...
  { nome: 'Pedro Lima', role: 'Cozinha · doces' },
] as const
...
export default function Dashboard() {
  ...
  const hidratado = useStorePedidos.persist?.hasHydrated() ?? true
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      ticket,
      ativos: ativos.length + 5, // 5 mock + ativos reais
    }
  }, [pedidos])
  ...
  const top5 = useMemo(() => {
    ...
    pedidos.forEach((p) =>
      p.itens.forEach((it) => {
        ...
        cur.receita += precoUnitario(it) * it.quantidade
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-6 py-6">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Visão geral · {unidadeAtual?.nome ?? 'Boa Viagem'} ·{' '}
      ...
      <ul className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <Kpi
          ...
          valor={stats.ativos.toString()}
          delta="cozinha em ritmo"
          variant="warning"
        />
      </ul>
