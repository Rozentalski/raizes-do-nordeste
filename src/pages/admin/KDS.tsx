...
import { precoUnitario } from '@/stores/carrinho'
import {
  ROTULOS_CANAL,
  ROTULOS_METODO,
  ROTULOS_MODO,
...
export default function KDS() {
  ...
  const atualizarStatus = useStorePedidos((s) => s.atualizarStatus)
  ...
  // Listener de storage event — quando outra aba modifica raizes_pedidos,
  // o Zustand persist do Zustand persist NÃO escuta mudanças de outras abas
  // por padrão. Forçamos um re-render fazendo "rehydrate" manual.
  useEffect(() => {
    function onStorage(e: StorageEvent) {
      if (e.key !== 'raizes_pedidos') return
      // Re-hydrata o estado a partir do localStorage atual
      void useStorePedidos.persist?.rehydrate?.()
    }
            ...
            <li key={it.itemId} className="text-sm">
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xs tabular-nums text-muted-foreground">
                  {(precoUnitario(it) * it.quantidade)
                    .toFixed(2)
                    .replace('.', ',')}
                </span>
              </div>
